...
if ~exist('results/sfm_caltech', 'dir');
    mkdir('results/sfm_caltech');
end

%% Options

repeats = 1; % repeat 20 individual trials
...
SAs = cell(6, 1);
for j = 1 : 6
    SAs{j} = zeros(length(datastr), repeats);
end
...
%         load(sprintf('results/sfm_caltech/%03d_%03d.mat', idm, idr));
...
%             angle6, idr);
        ...
        parfor_var_save9( ...
            sprintf('results/sfm_caltech/%03d_%03d.mat', idm, idr), ...
    ...
    % Store Result
    for idr = 1 : repeats
        tmp = load(sprintf('results/sfm_caltech/%03d_%03d.mat', idm, idr));
        for j = 1 : 6
    ...
    ms_path = sprintf('results/sfm_caltech/%s_svd.mat', datastr{idm});
...
% save('results/sfm_caltech/all_results.mat','SAs');
% 
% load('results/sfm_caltech/all_results.mat');
