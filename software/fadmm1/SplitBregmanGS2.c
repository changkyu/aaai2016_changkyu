/*				SplitBregmanGS2.c (MEX version) by Tom Goldstein
 * NOTE (Sejong Yoon, 2015/04/06): This code changed the original SplitBregmanROF.c file
 * so that it does a single Gauss-Seidel update and returns.
 *
 ...
 * 
 * [u, x, y, bx, by] = SplitBregmanGS2(lambdax, lambday, mu, f, u, x, y, bx, by);
 *
 *  lambdax : learning rate (rho.fx)
 *  lambday : learning rate (rho.fy)
 *  mu : mu value in total variation
 *  f : input image
 *  u : reconstructed image
 *  x, y : sparse variance
 *  bx, by : Bregman divergence
 *  
 */
...
void rof_iso(num** u, num** f, num** x, num** y, num** bx, num** by ,
        double mu, double lambdax, double lambday, int nGS, int nBreg, int width, int height);
...
void gs_an(num** u, num** f, num** x, num** y, num** bx, num** by , 
        double mu, double lambdax, double lambday, int width, int height, int iter);
void gs_iso(num** u, num** f, num** x, num** y, num** bx, num** by , 
        double mu, double lambdax, double lambday, int width, int height, int iter);
		
	/******************Relaxation Methods*****************/
void gsU(num** u, num** f, num** x, num** y, num** bx, num** by , 
        double mu, double lambdax, double lambday, int width, int height);
void gsX(num** u, num** x, num** bx , 
        double lambdax, double lambday, int width, int height);
void gsY(num** u, num** y, num** by , 
        double lambdax, double lambday, int width, int height);
void gsSpace(num** u, num** x, num** y, num** bx, num** by, 
        double lambdax, double lambday, int width, int height);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int rows = mxGetN(prhs[3]);
	int cols = mxGetM(prhs[3]);
	
	/* get the fidelity and convergence parameters*/
	double lambdax = (double)(mxGetScalar(prhs[0]));
	double lambday = (double)(mxGetScalar(prhs[1]));
	...
	double **f = get2dArray(prhs[3],1);
    double **u = get2dArray(prhs[4],1);
    double **x = get2dArray(prhs[5],1);
    double **y = get2dArray(prhs[6],1);
    double **bx = get2dArray(prhs[7],1);
    double **by = get2dArray(prhs[8],1);
    /*
...
//     double *outArray_by;
    ...
    int i,j;
     ...
     /***********Check Conditions******/  
    if (nrhs != 9) {mexErrMsgTxt("Seven input arguments required.");} 
    if (nlhs > 1){mexErrMsgTxt("Too many output arguments.");}
    if (!(mxIsDouble(prhs[3]))) {mexErrMsgTxt("f must be of type double.");}
    if (!(mxIsDouble(prhs[4]))) {mexErrMsgTxt("u must be of type double.");}
    if (!(mxIsDouble(prhs[5]))) {mexErrMsgTxt("x must be of type double.");}
    if (!(mxIsDouble(prhs[6]))) {mexErrMsgTxt("y must be of type double.");}
    if (!(mxIsDouble(prhs[7]))) {mexErrMsgTxt("bx must be of type double.");}
    if (!(mxIsDouble(prhs[8]))) {mexErrMsgTxt("by must be of type double.");}
...
// 	}while( (diff>tol && count<1000) || count<5 );
    ...
    gsU(u,f,x,y,bx,by,mu,lambdax,lambday,cols,rows);
    //gsX(u,x,bx,lambda,cols,rows);
    //gsY(u,y,by,lambda,cols,rows);    
    //bregmanX(x,u,bx,cols,rows);
    //bregmanY(y,u,by,cols,rows);
	...
	outArray_u = mxGetPr(plhs[0]);
	for(i=0;i<rows;i++){
	    for(j=0;j<cols;j++){
	        outArray_u[(i*cols)+j] = u[i][j];
	    }
	}

// 	plhs[1] = mxCreateDoubleMatrix(cols, rows, mxREAL); /*mxReal is our data-type*/
// 	outArray_x = mxGetPr(plhs[1]);
// 	for(i=0;i<rows-1;i++){
// 	    for(j=0;j<cols;j++){
// 	        outArray_x[(i*cols)+j] = x[i][j];
// 	    }
// 	}
// 
// 	plhs[2] = mxCreateDoubleMatrix(cols, rows, mxREAL); /*mxReal is our data-type*/
// 	outArray_y = mxGetPr(plhs[2]);
// 	for(i=0;i<rows;i++){
// 	    for(j=0;j<cols-1;j++){
// 	        outArray_y[(i*cols)+j] = y[i][j];
// 	    }
// 	}
// 
// 	plhs[3] = mxCreateDoubleMatrix(cols, rows, mxREAL); /*mxReal is our data-type*/
// 	outArray_bx = mxGetPr(plhs[3]);
// 	for(i=0;i<rows-1;i++){
// 	    for(j=0;j<cols;j++){
// 	        outArray_bx[(i*cols)+j] = bx[i][j];
// 	    }
// 	}
// 
// 	plhs[4] = mxCreateDoubleMatrix(cols, rows, mxREAL); /*mxReal is our data-type*/
// 	outArray_by = mxGetPr(plhs[4]);
// 	for(i=0;i<rows;i++){
// 	    for(j=0;j<cols-1;j++){
// 	        outArray_by[(i*cols)+j] = by[i][j];
// 	    }
// 	}
...
//         gs_iso(u,f,x,y,bx,by,mu,lambdax,lambday,width, height,nGS);
...
//     int j;
//     for(j=0;j<iter;j++){
//         gsU(u,f,x,y,bx,by,mu,lambdax,lambday,width,height);
//         gsSpace(u,x,y,bx,by,lambdax,lambday,width,height);
//     }
// }
...
//     int breg;
//     for(breg=0;breg<nBreg;breg++){
//         gs_an(u,f,x,y,bx,by,mu,lambdax,lambday,width, height,nGS);
//         bregmanX(x,u,bx,width,height);
//         bregmanY(y,u,by,width,height);
//     }
// }	
...
//     for(j=0;j<iter;j++){
//         gsU(u,f,x,y,bx,by,mu,lambdax,lambday,width,height);
//         gsX(u,x,bx,lambdax,lambday,width,height);
//         gsY(u,y,by,lambdax,lambday,width,height);
//     }
// }
...
void gsU(num** u, num** f, num** x, num** y, num** bx, num** by , 
	...
	int wm1,hm1;
    double normConst = 1.0/(mu+(lambdax*2+lambday*2));
	int wSent = width-1, hSent = height-1;
	for(w=1;w<wSent;w++){		/* do the central pixels*/
		wm1 = w-1;
		for(h=1;h<hSent;h++){
			hm1 = h-1;
            sumx = x[wm1][h] - x[w][h] - bx[wm1][h] + bx[w][h];
            sumy = y[w][hm1] - y[w][h] - by[w][hm1] + by[w][h];
            sumx += (u[w+1][h] + u[wm1][h]);
			sumy += (u[w][h+1] + u[w][hm1]);
			sumx *= lambdax;
            sumy *= lambday;
            sum = sumx + sumy;
			sum += (mu * f[w][h]);
	...
	for(h=1;h<hSent;h++){
		sumx = - x[w][h] + bx[w][h];
        sumy = y[w][h-1] - y[w][h] -by[w][h-1] + by[w][h];
        sumx += u[w+1][h];
		sumy += (+u[w][h+1]+u[w][h-1]);
        sumx *= lambdax;
        sumy *= lambday;
		sum = sumx + sumy;
		sum += (mu * f[w][h]);
        sum /= (mu+(lambdax+2*lambday));
		u[w][h] = sum;
	}
	w = width-1;		/* do the right pixels*/
	for(h=1;h<hSent;h++){
        sumx = x[w-1][h] - bx[w-1][h];
        sumy = y[w][h-1] - y[w][h] - by[w][h-1] + by[w][h];
        sumx += u[w-1][h];
        sumy += (u[w][h+1] + u[w][h-1]);
		sumx *= lambdax;
	...
	for(w=1;w<wSent;w++){		/* do the top pixels*/
        sumx = x[w-1][h] - x[w][h] - bx[w-1][h] + bx[w][h];
        sumy = - y[w][h] + by[w][h];
        sumx += u[w+1][h] + u[w-1][h];
		sumy += u[w][h+1];
        sumx *= lambdax;
        sumy *= lambday;
		sum = sumx + sumy;
		sum += (mu * f[w][h]);
        sum /= (mu+(2*lambdax+lambday));
		u[w][h] = sum;
	}
	h = height-1;
	for(w=1;w<wSent;w++){		/* do the bottom pixels*/
        sumx = x[w-1][h] - x[w][h] - bx[w-1][h] + bx[w][h];
		sumy = y[w][h-1] - by[w][h-1];
        sumx += u[w+1][h]+u[w-1][h];
		sumy += u[w][h-1];
        sumx *= lambdax;
        sumy *= lambday;
	...
	w=h=0;
	sumx = - x[w][h] + bx[w][h] + u[w+1][h];
    sumy = - y[w][h] + by[w][h] + u[w][h+1];
	sumx *= lambdax;
    sumy *= lambday;
    sum = sumx + sumy;
	sum += (mu * f[w][h]);
    sum /= (mu+(lambdax+lambday));
	u[w][h] = sum;
	/* do the top-right pixel*/		
	w=width-1; h=0;
	sumx = x[w-1][h] - bx[w-1][h] + u[w-1][h];
    sumy = - y[w][h] + by[w][h] + u[w][h+1];
	sumx *= lambdax;
    sumy *= lambday;
    sum = sumx + sumy;
...
// void gsSpace(num** u, num** x, num** y, num** bx, num** by, 
//         double lambdax, double lambday, int width, int height){
// 	double lambda = (lambdax + lambday)/2;
//     int w,h;
// 	num a,b,s;
// 	num flux = 1.0/lambda;
