/*				SplitBregmanGS.c (MEX version) by Tom Goldstein
 ...
 * 
 * [u, x, y, bx, by] = SplitBregmanGS(lambda, mu, f, u, x, y, bx, by);
 *
 *  lambda : learning rate (rho in Boyd, et al.)
...
void gs_an(num** u, num** f, num** x, num** y, num** bx, num** by , 
        double mu, double lambda, int width, int height, int iter);
void gs_iso(num** u, num** f, num** x, num** y, num** bx, num** by , 
        double mu, double lambda, int width, int height, int iter);
		
	/******************Relaxation Methods*****************/
void gsU(num** u, num** f, num** x, num** y, num** bx, num** by , 
        double mu, double lambda, int width, int height);
void gsX(num** u, num** x, num** bx , 
        double lambda, int width, int height);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int rows = mxGetN(prhs[2]);
	int cols = mxGetM(prhs[2]);
	...
	double **f = get2dArray(prhs[2],1);
    double **u = get2dArray(prhs[3],1);
    double **x = get2dArray(prhs[4],1);
    double **y = get2dArray(prhs[5],1);
    double **bx = get2dArray(prhs[6],1);
    double **by = get2dArray(prhs[7],1);
    /*
...
//     double *outArray_by;
    ...
    int i,j;
     ...
     /***********Check Conditions******/  
    if (nrhs != 8) {mexErrMsgTxt("Seven input arguments required.");} 
    if (nlhs > 1){mexErrMsgTxt("Too many output arguments.");}
    if (!(mxIsDouble(prhs[2]))) {mexErrMsgTxt("f must be of type double.");}
    if (!(mxIsDouble(prhs[3]))) {mexErrMsgTxt("u must be of type double.");}
    if (!(mxIsDouble(prhs[4]))) {mexErrMsgTxt("x must be of type double.");}
    if (!(mxIsDouble(prhs[5]))) {mexErrMsgTxt("y must be of type double.");}
    if (!(mxIsDouble(prhs[6]))) {mexErrMsgTxt("bx must be of type double.");}
    if (!(mxIsDouble(prhs[7]))) {mexErrMsgTxt("by must be of type double.");}
...
// 	}while( (diff>tol && count<1000) || count<5 );
    ...
    gsU(u,f,x,y,bx,by,mu,lambda,cols,rows);
...
void gsU(num** u, num** f, num** x, num** y, num** bx, num** by , 
	...
	for(w=1;w<wSent;w++){		/* do the central pixels*/
		...
		for(h=1;h<hSent;h++){
			hm1 = h-1;
			sum = x[wm1][h] - x[w][h] - bx[wm1][h] + bx[w][h] 
                    + y[w][hm1] - y[w][h] - by[w][hm1] + by[w][h];
